...
def new_test(test: Test, credentials: HTTPBasicCredentials = Depends(security)):
    if check_cred(users_db, credentials):
        if test.nombre not in [5, 10, 20]:
            raise HTTPException(
                400, "You can only choose 5, 10 or 20 questions")
        string = "".join(test.subjects)
        data = df.copy()
        data["is_in"] = data["subject"].apply(lambda x: x in string)
        data = data.loc[(data["is_in"]) & (data["use"] == test.use)]
        try:
            data = data.drop(["is_in", "correct", "subject", "use"], axis=1).sample(
                test.nombre).T.to_json()
        except ValueError:
            raise HTTPException(
                400, "Cannot create a new test of {} questions. Change your filters".format(test.nombre))
        return data
